...
export function SerialSearch({ machines, autoFocus = false }: { machines: Machine[]; autoFocus?: boolean }) {
  ...
  function go(value?: string) {
    ...
    if (!query) return;
    const hit = machines.find((m) => m.serial?.toLowerCase() === query);
    ...
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-2 bg-pos-surface2 border border-pos-line rounded-md px-3 py-1.5">
        <input
          ...
          placeholder="Scan or search serial…"
